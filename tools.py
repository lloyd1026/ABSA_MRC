...
def pair_combine(forward_pair_list, forward_pair_prob, forward_pair_ind_list,
    ...
    for idx in range(len(forward_pair_list2)):
        if forward_pair_list2[idx] not in backward_pair_list2 and forward_pair_prob2[idx] < alpha:
            continue  # 如果只有单条路径认可 并且认可度低于alpha，将此结果丢弃
        else:
            if forward_pair_list2[idx][0] not in final_asp_list:
                ...
                final_asp_ind_list.append(forward_pair_ind_list2[idx][:2])  # 将aspect的[start_id, end_id] 切片出来
                final_opi_ind_list.append([forward_pair_ind_list2[idx][2:]])  # 将opinion的[start_id, end_id]切片出来
            else:
                asp_index = final_asp_list.index(forward_pair_list2[idx][0])  # 获得final中记录的这个aspect的下标
                if forward_pair_list2[idx][1] not in final_opi_list[asp_index]:
                    # aspect 已经在final_aspect_list中了，但是opinion没有
                    # pair是一对一的关系 aspect可能会对应多个opinion
                    final_opi_list[asp_index].append(forward_pair_list2[idx][1])
                    final_opi_ind_list[asp_index].append(forward_pair_ind_list2[idx][2:])  # 补充opinion的index
    # backward
    for idx in range(len(backward_pair_list2)):
        if backward_pair_list2[idx] not in forward_pair_list2 and backward_pair_prob2[idx] >= alpha:
...
def triplet_combine(forward_triplet_list, forward_triplet_prob, forward_triplet_idx_list,
    ...
    for idx in range(len(forward_triplet_list2)):
        ...
        if forward_triplet_list2[idx][0] not in final_asp_list:
            # 新的aspect
            final_asp_list.append(forward_triplet_list2[idx][0])
            final_opi_list.append([forward_triplet_list2[idx][1]])
            final_adv_list.append([[forward_triplet_list2[idx][2]]])
            ...
            if forward_triplet_list2[idx][1] not in final_opi_list[asp_index]:
                # opi 没有
                final_opi_list[asp_index].append(forward_triplet_list2[idx][1])
                final_opi_idx_list[asp_index].append(forward_triplet_idx_list2[idx][2:4])
                # final_adv_list:索引到对应asp的opi区域，在opi区域中加入一个新的二级列表
                final_adv_list[asp_index].append([forward_triplet_list2[idx][2]])
    ...
    # forward 已将共同存在的处理了  剩余的就是 forward没有 但是backward有
    # backward
    for idx in range(len(backward_triplet_list2)):
        if backward_triplet_list2[idx] not in forward_triplet_list2:
            ...
            if backward_triplet_list2[idx][0] not in final_asp_list:
                final_asp_list.append(backward_triplet_list2[idx][0])
                final_opi_list.append([backward_triplet_list2[idx][1]])
                final_adv_list.append([[backward_triplet_list2[idx][2]]])
                ...
                if backward_triplet_list2[idx][1] not in final_opi_list[asp_index]:
